#! -*- coding: utf-8 -*- \\\sample code - usinng IMMDB data - delta is setting to 2\\
...
class My_LSTM(Layer):
    ...
    def build(self, input_shape): # parameter
    ...
    def step_do(self, step_in, states): # each step
...
c_s =Lambda(lambda x: x[:,:,:emb])(ch)
h = Lambda(lambda x: x[:,:,emb:])(ch)
